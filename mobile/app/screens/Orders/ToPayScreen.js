...
const ToPayScreen = () => {
    ...
    return (
        <ScrollView className="p-4">
            {orders.map((order) => (
                <View key={order.id} className="bg-yellow-100 p-4 mb-4 rounded-lg flex-row items-center justify-between">
                    <View>
                        <Text className="text-lg font-bold">{order.item}</Text>
                        <Text className="text-gray-600">Total: {order.total}</Text>
                        <Text className="text-yellow-600">Due by: {order.dueDate}</Text>
                    </View>
                    <Icon name="card" size={24} color="#ffcc00" className="ml-2" />
                </View>
            ))}
        </ScrollView>
    )
