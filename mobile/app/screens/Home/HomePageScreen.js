...
function HomePageScreen({ route }) {
  ...
  const fetchCrops = async () => {
    try {
      ...
      const crops = rawcrops.filter(
        (crop) => crop.availability === "live" && crop.crop_quantity > 0
      );
      ...
      const combinedData = filteredCrops.map((crop) => {
        ...
        const actualSize = sizes.find(
          (size) => size.crop_size_id === (crop ? crop.crop_size_id : null)
        );
      ...
      const filterCrops = combinedData.filter((crop) => {
        const matchesCategory = filter_category_id
          ? crop.category_id === Number(filter_category_id)
          : true;
        const matchesSubCategory = filter_sub_category_id
          ? crop.sub_category_id === Number(filter_sub_category_id)
          : true;
        const matchesVariety = filter_variety_id
          ? crop.variety?.crop_variety_id === Number(filter_variety_id)
          : true;
        const matchesSize = filter_size_id
          ? crop.crop_size_id === Number(filter_size_id)
          : true;
        const matchesQuantity =
          filter_quantity && filter_quantity[0] !== undefined
            ? crop.crop_quantity >= Number(filter_quantity[0])
            : true;
        const matchesPrice =
          filter_price_range && filter_price_range[0] !== undefined
            ? crop.crop_price >= Number(filter_price_range[0])
            : true;
        const matchesClass =
          filter_class && filter_class.length > 0
  ...
  const handleSearch = (query) => {
    ...
    if (query.trim() === "") {
      ...
      const searchResults = combinedData.filter(
        (crop) =>
          ...
          (crop.crop_rating &&
            crop.crop_rating.toString().includes(lowercasedQuery)) ||
          (crop.crop_quantity &&
            crop.crop_quantity.toString().includes(lowercasedQuery)) ||
  ...
  const sortOptions = [
    {
      method: "price",
      iconAsc: "sort-amount-down-alt",
      iconDesc: "sort-amount-up",
    },
    {
      method: "rating",
      iconAsc: "sort-amount-down-alt",
  ...
  const handleSort = (sortBy, order) => {
    ...
    if (sortBy === "price") {
      sortedData = [...filteredData].sort((a, b) =>
        order === "asc"
          ? a.crop_price - b.crop_price
          : b.crop_price - a.crop_price
      );
    } else if (sortBy === "rating") {
      sortedData = [...filteredData].sort((a, b) =>
        order === "asc"
          ? a.crop_rating - b.crop_rating
          : b.crop_rating - a.crop_rating
      );
    } else if (sortBy === "available") {
      sortedData = [...filteredData].sort((a, b) =>
        order === "asc"
          ? a.crop_quantity - b.crop_quantity
          : b.crop_quantity - a.crop_quantity
      );
    } else if (sortBy === "class") {
      sortedData = [...filteredData].sort((a, b) =>
        order === "asc"
          ? a.crop_class.localeCompare(b.crop_class)
          : b.crop_class.localeCompare(a.crop_class)
      );
    }
  ...
  return (
    <SafeAreaView className="flex-1 bg-gray-50">
      <View className="bg-gray-100 sticky top-0 z-10 pt-2 mb-2">
      ...
      <View className="p-4 py-1 flex-row items-center space-x-3 bg-white shadow-sm">
        ...
        <TouchableOpacity
          ...
          onPress={() =>
            navigation.push("Filter Products", {
              filter_category_id,
        ...
        <View className="flex-1 justify-center items-center bg-black/50">
          <View className="bg-white rounded-lg shadow-lg p-6 w-4/5">
                ...
                <View className="flex-row justify-between">
                  <TouchableOpacity
                    ...
                    className={`flex-1 p-3 rounded-lg border ${
                      currentSort.method === option.method &&
                      currentSort.order === "asc"
                        ? "bg-green-300"
                        : "bg-white"
                    } border-gray-300 flex-row items-center justify-center shadow-sm`}
                  >
                    <Icon
                      name={option.iconAsc}
                      size={16}
            ...
            <Pressable
              ...
              className="mt-6 p-3 bg-[#00B251] rounded-md shadow-md"
            >
        ...
        <View className="px-4">
          <View className="space-y-3 mb-12">
            {filteredData.length > 0 ? (
              filteredData
                .filter(
                  (crop) => !shopData || shopData.shop_id !== crop.shop_id
                )
                .map((crop) => (
      ...
      {shopData && (
        <TouchableOpacity
          className="absolute flex-row items-center bottom-6 right-5 bg-[#00B251] rounded-full p-4 shadow-lg"
...
const ShopCard = ({
  ...
  available = 0,
  rating = 0,
  shopImage = placeholderimg,
  cropDescription = "",
  metricSymbol = "",
  size = "",
  cropClass = "",
  cropImage = placeholderimg,
  shop_id,
      ...
      <View className="flex-1 justify-between">
        ...
        >
          <View className="w-10 h-10 rounded-full overflow-hidden border border-gray-300">
            <Image
              source={shopImage ? { uri: shopImage } : placeholderimg}
              className="w-full h-full"
            />
          </View>
          <Text className="text-base ml-3 font-semibold text-gray-700">
            {shopName}
          </Text>
        </Pressable>
        <Pressable
          onPress={() =>
            navigation.navigate("Product Details", { product: cropData })
          }
        >
          <Text className="text-lg text-center font-extrabold text-[#00B251]">
            {productName} ({size})
          </Text>
          <View className="flex-1 flex-row">
            <View className="w-20 h-20 overflow-hidden rounded-lg">
              <Image
                source={cropImage ? { uri: cropImage } : placeholderimg}
                className="w-full h-full"
              />
            </View>
              ...
              <View className="w-2/5">
                ...
                {negotiationAllowed ? (
                  <Text className="text-sm text-green-600 font-extrabold">
                    {minimumNegotiation} {metricSymbol}
                  </Text>
                ) : (
                  ""
                )}
            ...
            <View className="justify-center w-2/5">
              <Text className="text-2xl text-center font-bold text-[#00B251]">
                {" "}
                ₱{price}/{metricSymbol}
              </Text>
            </View>
