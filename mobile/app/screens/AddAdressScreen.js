...
function AddAddressScreen({ route, navigation, onLocationSelect: handleLocationSelect = () => {} }) {
      ...
      <ScrollView contentContainerStyle={{ flexGrow: 1 }}>
        ...
        <View className="p-5">
          ...
          <View className="flex-wrap flex-row justify-between mb-5">
            <TouchableOpacity className={`flex-row items-center border ${label === 'Home' ? 'bg-green-600' : 'border-green-600'} rounded-lg py-2 px-3 m-1`} onPress={() => handleLabelSelect('Home')}>
              <Icon name="home" type="font-awesome" size={20} color={label === 'Home' ? 'white' : '#00B251'} />
              <Text className={`ml-2 ${label === 'Home' ? 'text-white' : 'text-green-600'}`}>Home</Text>
            </TouchableOpacity>
            <TouchableOpacity className={`flex-row items-center border ${label === 'Work' ? 'bg-green-600' : 'border-green-600'} rounded-lg py-2 px-3 m-1`} onPress={() => handleLabelSelect('Work')}>
              <Icon name="briefcase" type="font-awesome" size={20} color={label === 'Work' ? 'white' : '#00B251'} />
              <Text className={`ml-2 ${label === 'Work' ? 'text-white' : 'text-green-600'}`}>Work</Text>
            </TouchableOpacity>
            <TouchableOpacity className={`flex-row items-center border ${label === 'Partner' ? 'bg-green-600' : 'border-green-600'} rounded-lg py-2 px-3 m-1`} onPress={() => handleLabelSelect('Partner')}>
              <Icon name="heart" type="font-awesome" size={20} color={label === 'Partner' ? 'white' : '#00B251'} />
              <Text className={`ml-2 ${label === 'Partner' ? 'text-white' : 'text-green-600'}`}>Partner</Text>
            </TouchableOpacity>
            <TouchableOpacity className={`flex-row items-center border ${label === 'Other' ? 'bg-green-600' : 'border-green-600'} rounded-lg py-2 px-3 m-1`} onPress={() => handleLabelSelect('Other')}>
              <Icon name="plus" type="font-awesome" size={20} color={label === 'Other' ? 'white' : '#00B251'} />
              <Text className={`ml-2 ${label === 'Other' ? 'text-white' : 'text-green-600'}`}>Other</Text>
            </TouchableOpacity>
          </View>
          <TouchableOpacity
            className="bg-green-600 py-4 rounded-full items-center"
            onPress={() => {
              Alert.alert('Address Saved', `Address: ${address}, Note: ${note}, Label: ${label}`);
            }}
          >
