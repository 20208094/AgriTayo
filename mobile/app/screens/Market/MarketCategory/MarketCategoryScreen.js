...
const CategoryItemCard = ({ item }) => {
  ...
  return (
    ...
    >
      <Image
        source={
          item.crop_image_url ? { uri: item.crop_image_url } : placeholderimg
        }
      ...
      <View className="p-4">
        ...
        <Text className="text-sm text-gray-600 mt-1">
          Size: {item.size.crop_size_name}
        </Text>
        <Text className="text-sm text-gray-600 mt-1">
          Class: {item.crop_class}
        </Text>
        <Text className="text-sm text-gray-600 mt-1">
          {item.crop_description}
        </Text>
...
function MarketCategoryScreen({ route }) {
  ...
  ); // Initialize state with route parameter
    ...
    try {
      ...
      if (selectedSubCategories.length > 0) {
        const filteredCrops = data.filter((crop) =>
          selectedSubCategories.includes(crop.sub_category_id)
        );
    ...
    async (selectedItemId) => {
      try {
        ...
        const filteredCrops = data.filter(
          (crop) => crop.crop_variety_id === selectedItemId
        );
