...
const MarketAnalyticsPage = () => {
  ...
  const { category, selectedItemId } = location.state || {};
  const [selectedFilter, setSelectedFilter] = useState('12 Months');
  const [modalVisible, setModalVisible] = useState(false);

  // Find the selected item based on its ID
  const selectedItem = category.find((item) => item.id === selectedItemId);
      ...
      {selectedItem && (
        <div className="bg-white p-4 rounded-lg shadow-md mb-6">
          ...
          <p className="text-sm font-bold text-green-500 mb-2">
            ...
            <span className="text-green-700">30 Listings</span>
          </p>
          <p className="text-sm font-bold text-green-500 mb-2">
            Current Highest Price/Kilo:{" "}
            <span className="text-green-700">₱70/kilo</span>
          </p>
          <p className="text-sm font-bold text-green-500 mb-4">
            Current Lowest Price/Kilo:{" "}
            <span className="text-green-700">₱50/kilo</span>
          </p>
