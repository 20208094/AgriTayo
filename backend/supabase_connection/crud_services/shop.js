...
async function addShop(req, res) {
  try {
    ...
    form.parse(req, async (err, fields, files) => {
      ...
      const secondary_shop_number = getSingleValue(
        fields.secondary_shop_number
      );
...
async function updateShop(req, res) {
  ...
  try {
    ...
    form.parse(req, async (err, fields, files) => {
      ...
      if (fetchError1) {
        console.error(
          "Failed to fetch current shop data:",
          fetchError1.message
        );
        return res.status(500).json({ error: "Internal server error" });
      }
...
async function editShopPhoneNumber(req, res) {
  try {
    ...
    if (!shop_id) {
      return res.status(400).json({ error: "Shop Id is required for update" });
    }
    ...
    form.parse(req, async (err, fields, files) => {
      ...
      const { edit_phone_number } = fields;
      ...
      console.log(edit_phone_number, shop_id);
...
async function editSecondaryShopPhoneNumber(req, res) {
  try {
    ...
    form.parse(req, async (err, fields, files) => {
      ...
      const { edit_secondary_phone_number } = fields;
      ...
      console.log(edit_secondary_phone_number, shop_id);
