...
async function getBids(req, res) {
  try {
    const { data, error } = await supabase.from("biddings").select("*");
...
async function addBid(req, res) {
  try {
    ...
    form.parse(req, async (err, fields, files) => {
      ...
      const bidCategoryId = parseInt(getSingleValue(bid_category_id), 10);
      ...
      const bidMinimumIncrement = parseFloat(
        getSingleValue(bid_minimum_increment)
      );
      ...
      const { data, error } = await supabase.from("biddings").insert([
        {
          shop_id: shopId,
          ...
          metric_system_id: metricSystemId,
  ...
  try {
    ...
    form.parse(req, async (err, fields, files) => {
      ...
      const updateData = {
        ...
        bid_category_id: parseInt(getSingleValue(bid_category_id), 10),
        metric_system_id: parseInt(getSingleValue(metric_system_id), 10),
        bid_starting_price: parseFloat(getSingleValue(bid_starting_price)),
        bid_minimum_increment: parseFloat(
          getSingleValue(bid_minimum_increment)
        ),
...
async function deleteBid(req, res) {
  try {
    ...
    if (fetchError) {
      ...
      return res
        .status(500)
        .json({ error: "Failed to fetch bid for deletion" });
    }
    ...
    if (error) {
      ...
      return res
        .status(500)
        .json({ error: "Failed to delete bid from database" });
    }
